...
const OrderedViewPagePending = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        // Filter orders to include only those with OrderStatus = PrepareProducts (enum value 0)
  ...
  }, [orders, accessToken]); // Re-run if orders or accessToken changes
  ...
  return (
    <div className="container mx-auto mt-8 p-4">
      <h1 className="text-2xl font-bold mb-6">My Orders (Pending)</h1>
          ...
          <tbody>
            {orders.map((order) => (
              <tr key={order.orderId}>
                ...
                <td className="border px-4 py-2">${order.totalPrices.toFixed(2)}</td>
                <td className="border px-4 py-2">Preparing Products</td>
                <td className="border px-4 py-2">
                ...
                <td className="border px-4 py-2">{order.paymentMethod.name}</td>
                <td className="border px-4 py-2">
                  <ul>
...
export default OrderedViewPagePending;
